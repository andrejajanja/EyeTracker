...
kamera = Kamera(0.0,4.0,-15.0,90.0,0.25, Vector3([0.0,0.0,1.0]), Vector3([0.0,0.0,1.0]))
...
brojac = 0
radi = False
fov = 80 #stepeni za inicijalni field of view
...
f = open("resursi/kalibracija.txt", "w")
...
def pritisak_dugmeta(prozor, dugme, scancode, akcija, mode):
    global napred, nazad, levo, desno, radi
    ...
    if dugme == glfw.KEY_D and akcija == glfw.PRESS:
        desno = True
    if dugme == glfw.KEY_E and akcija == glfw.PRESS:
        radi = True
...
prozor = glfw.create_window(sirina, visina, "E za kalibraciju", None, None)
...
def slika_brojac(brojac, detector, predictor, z_ploca):
    slika = cv2.imread("kal_{}.jpg".format(brojac))
    slika, niz = provuci_kroz_ai(detector, predictor, slika, z_ploca)
    return True, slika, niz
...
while not glfw.window_should_close(prozor):
    ...
    radi, slika, niz = slika_brojac(brojac, detector, predictor, z_ploca)
    ...
    #pomocna sfera
    pomoc = pozicija_rotacija_razmera_modela(P = Vector3(tackice_monitor[brojac]), S = 0.015)   
    pozovi_ucitan_model_opengl(VAO[2], tekstura[2], pomoc, lokacija_modela, indeksi_sfera, GL_TRIANGLES)

    nacrtaj_sferu_na_monitoru(niz, 1, z_ploca, z_monitora, VAO[2], lokacija_modela, tekstura[2], indeksi_sfera)
    nacrtaj_sferu_na_monitoru(niz, 4, z_ploca, z_monitora, VAO[2], lokacija_modela, tekstura[2], indeksi_sfera)

    radi, brojac = upisi_offsete(niz, 1, 4, z_ploca, z_monitora, brojac, radi, prozor, f)
