...
kamera = Kamera(-30.0,0.0,-10.0,0,0.25)
sirina,visina = 1280,720
poslednjex, poslednjey = sirina/2 , visina/2
prvi_mis = True
fov = 45 #stepeni za inicijalni field of view
...
#ucitavanje tracaka i indeksa modela
tacke_monitor, indeksi_monitor = Ucitavanje_obj.ucitaj_obj("resursi/{}.obj".format(ime1))
tacke_ploca, indeksi_ploca = Ucitavanje_obj.ucitaj_obj("resursi/{}.obj".format(ime2))
...
VBO = glGenBuffers(2)
tekstura = glGenTextures(2)
#endregion inicijalizacije svega

#ucitavanje modela i njihovih tekstura
ucitaj_model_opengl(VAO[0],VBO[0], tacke_monitor)
monitor_tex = ucitaj_texturu(tekstura[0], "resursi/{}Color.jpg".format(ime1))

ucitaj_model_opengl(VAO[1], VBO[1],tacke_ploca)

#region jos inicijalizacije
glUseProgram(shader)
...
ploca_poz_modela = pozicija_rotacija_razmera_modela(P = Vector3([0.0,0.0,-10.0]))
...
while not glfw.window_should_close(prozor):
    ...
    pozovi_ucitan_model_opengl_trouglovi(VAO[0], tekstura[0], monitor_poz_modela, lokacija_modela, indeksi_monitor)
    pozovi_ucitan_model_opengl_quads(VAO[1], tekstura[1],ploca_poz_modela,lokacija_modela,indeksi_ploca)
